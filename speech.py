...
@@ -69,8 +70,10 @@ def __init__(self):
        ...
        ]) # use a set to avoid duplicate voice entries
        self.current_kokoro_voice = 'af_heart'
        self.current_language = 'English'
        #default language matches setup_kokoro() which initializes with lang_code='a'

        self._cb = {}
        for cb in ['peak', 'wave', 'idle']:
@@ -102,6 +105,16 @@ def set_kokoro_voice(self, voice_name):
        else:
            logger.warning(f"Invalid Kokoro voice: {voice_name}.")
    def set_language(self, language_name):
    """Set the active Kokoro TTS language by name."""
